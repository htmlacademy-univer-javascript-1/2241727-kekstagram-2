import {insertPhotoMiniature, clearPhotoMiniature} from './miniatures.js';
import {debounce, getRandomPositiveNumber} from './util.js';
...
function tenRandomSort() {
  ...
  const used = new Set();
  while (used.size < 10) {
    const i = getRandomPositiveNumber(0, photos.length - 1);
    if (used.has(i)) {
      continue;
    }
    used.add(i);
